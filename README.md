...
1. Откройте **амоМаркет → Интеграции** в аккаунте amoCRM.
2. Нажмите **Создать интеграцию** и выберите подходящий тип интеграции.
...
6. На вкладке ключей нажмите **«Сгенерировать токен»**, выберите срок и
   скопируйте значение целиком (длинная строка вида `eyJ0eXAi...`). Такой
   токен действует от 1 дня до 5 лет, не имеет refresh token и после истечения
   либо отзыва должен быть сгенерирован заново.
